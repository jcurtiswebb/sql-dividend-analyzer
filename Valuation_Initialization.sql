...
CREATE PROC prd.Valuation
	@rateaboveriskfree AS Decimal(6,4),
	@riskfree AS Decimal(6,4)
AS
BEGIN
	--If percentages were given instead of decimals, divide percentage by 100
		...
		IF @rateaboveriskfree > 0
			...
			END
	UPDATE N2
		SET N2.Valuation = ROUND((N1.div_growth+1),3)*ROUND(N1.dividend,3)/((@riskfree + @rateaboveriskfree)*N1.Beta - N1.div_growth)
